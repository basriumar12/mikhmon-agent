...
function purchaseDigiflazz($phone, $sku, $customerNo) {
    ...
    try {
        ...
        // Execute Digiflazz transaction (no processing message, wait for final result)
        ...
        sendDigiflazzResult($phone, $product, $customerNo, $sellPrice, $serialNumber, $status, $message, $isAdmin, $balanceAfter);
 ...
 */
function sendDigiflazzResult($phone, $product, $customerNo, $price, $serialNumber, $status, $message, $isAdmin, $balanceAfter) {
    $statusIcon = '✅';
    $statusText = 'SUKSES';
    ...
    $resultMsg .= "━━━━━━━━━━━━━━━━━━━━\n";
    $resultMsg .= "📦 *Produk*\n";
    $resultMsg .= "   {$product['product_name']}\n";
    $resultMsg .= "   Brand: {$product['brand']}\n\n";
    $resultMsg .= "📱 *Nomor Tujuan*\n";
    $resultMsg .= "   {$customerNo}\n\n";
    $resultMsg .= "💰 *Harga*\n";
    $resultMsg .= "   Rp " . number_format($price, 0, ',', '.') . "\n\n";
    
    if (!empty($serialNumber)) {
        $resultMsg .= "🔢 *Serial Number*\n";
        $resultMsg .= "   `{$serialNumber}`\n\n";
    }
    
    $resultMsg .= "📊 *Status*\n";
    $resultMsg .= "   " . strtoupper($status) . "\n";
    ...
    if (!$isAdmin && $balanceAfter > 0) {
        $resultMsg .= "\n💳 *Saldo Anda*\n";
        $resultMsg .= "   Rp " . number_format($balanceAfter, 0, ',', '.') . "\n";
    }
