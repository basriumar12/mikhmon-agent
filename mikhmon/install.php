...
function logMessage($message, $type = 'info') {
    $icon = ['success' => '✅', 'error' => '❌', 'warning' => '⚠️', 'info' => 'ℹ️'][$type] ?? 'ℹ️';
...
function ensureTable($pdo, $name, $ddl) {
    if (!tableExists($pdo, $name)) {
        ...
        if (strpos($ddl, 'CONSTRAINT') !== false && strpos($ddl, 'FOREIGN KEY') !== false) {
            ...
            $ddlWithoutFK = preg_replace('/,?\s*CONSTRAINT\s+`[^`]+`\s+FOREIGN\s+KEY\s*\([^)]+\)\s+REFERENCES\s+`[^`]+`\s*\([^)]+\)(\s+ON\s+(DELETE|UPDATE)\s+(CASCADE|SET\s+NULL|NO\s+ACTION|RESTRICT))*\s*/i', '', $ddl);
            // Clean up double commas and trailing commas before closing parenthesis
            $ddlWithoutFK = preg_replace('/,\s*,/', ',', $ddlWithoutFK); // Remove double commas
            $ddlWithoutFK = preg_replace('/,\s*\)/', ')', $ddlWithoutFK); // Remove trailing comma before )
...
<div class="container fade-in">
    ...
    <div class="content">
        ...
        <?php if ($step == 'config'): ?>
            <div class="fade-in">
                <h2 style="margin-bottom: 20px; font-size: 20px;">⚙️ Konfigurasi Database</h2>
                <?= $configMessage ?>
        ...
        <?php elseif ($step == 'check'): ?>
            <div class="fade-in">
                ...
                <?php else: ?>
                    <h2 style="margin-bottom: 20px; font-size: 20px;">📊 Status Sistem</h2>
        ...
        <?php elseif ($step == 'install'): ?>
            <div class="fade-in">
                ...
                <?php else: ?>
                    <h2 style="margin-bottom: 20px; font-size: 20px;">🚀 Proses Instalasi</h2>
                    <div class="log-area">
                        <?php
